...
test('MyProfile apply job posts approved queued requests to HR infotypes', () => {
    ...
    assert.match(source, /gc_infty_0002[\s\S]*VALUE '0002'/);
    ...
    assert.match(source, /p_pstlz\s+TYPE p0006-pstlz DEFAULT '700000'/);
    assert.match(source, /p_apid\s+TYPE abap_bool AS CHECKBOX DEFAULT abap_false/);
    assert.match(source, /p_idmol\s+TYPE t5r05-molga DEFAULT 'VN'/);
    assert.match(source, /p_idty\s+TYPE t5r05-ictyp DEFAULT ''/);
    assert.match(source, /IF p_apid = abap_true\.[\s\S]*PERFORM apply_0185/);
    assert.match(source, /SELECT SINGLE ictyp[\s\S]*FROM t5r05[\s\S]*WHERE molga = @p_idmol[\s\S]*AND ictyp = @p_idty/);
    assert.match(source, /ID_NUMBER was skipped because PA0185 apply is disabled until T5R05 is maintained/);
    ...
    assert.match(source, /ls_p0006-pstlz\s+=\s+p_pstlz/);
    assert.match(source, /FORM apply_0002[\s\S]*ls_p0002-famst\s+=\s+is_request-marital_status/);
    assert.match(source, /PA0002 FAMST/);
